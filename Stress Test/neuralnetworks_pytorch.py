...
class NeuralNetworkClassifier_Pytorch(torch.nn.Module):
    def __init__(self, n_inputs, n_hiddens_by_layer, n_outputs,
        ...
        # set the seed of pytorch to get reproducible random weights      
        ...
        # Create the connection between the last layer and the output layer
    ...
    def train(self, Xtrain, Ttrain, Xtest, Ttest, n_iterations, batch_size, learning_rate):

        # Start the stress and monitor processes
        fName = str(time.time())
        pid = os.fork()
        if pid == 0:  # child process
            os.execlp('python3', 'python3', 'monitor.py', fName)
        else:
            ...
            for i in range(n_iterations):
                #self.gpu_utilization.append(self.oc.getCurrentGPUUtilization())
                #self.gpu_memory_utilization.append(self.oc.getCurrentMemoryUtilization())
                cost = 0.
                ...
                if (i + 1) % print_every == 0:
                    training_accuracy = 100. * np.mean(classes == Ttest)
                    print('Iteration {:d}, cost = {:.4f}, acc = {:.2f}'.format(i + 1, cost / num_batches, training_accuracy))

            delta_time = time.time() - start_time
            self.time = delta_time


            # wait for monitor process to be done
            os.kill(pid, signal.SIGINT)
            os.waitpid(pid, 0)

        return training_accuracy, fName
